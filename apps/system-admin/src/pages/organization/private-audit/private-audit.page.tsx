...
import { privateAuditMock } from '@/shared/mock/private-audit.data'

import { PrivateAuditEdit } from './components/private-audit-edit.dialog'

export default function PrivateAudit() {
  const { id } = useParams()
  const privateAudit = privateAuditMock.find((org) => org.id === Number(id))
  if (!privateAudit) return null
      ...
      <Breadcrumb className="mb-4">
        <BreadcrumbList>
          ...
          <BreadcrumbItem className="uppercase">
            <BreadcrumbPage>{privateAudit.full_name}</BreadcrumbPage>
          </BreadcrumbItem>
        </BreadcrumbList>
      </Breadcrumb>
        ...
        <CardHeader className=" flex items-center font-medium  ">
          <div className="flex items-center justify-between w-full ">
            <CardTitle className="text-xl  flex items-center gap-1.5 uppercase  font-medium ">
              <img
                src={privateAudit.logo || '/placeholder.svg'}
                alt="LOGO"
                className="h-10 w-10 object-contain "
              />
              <p style={{ color: privateAudit.primary_color }}>
                {privateAudit?.full_name}
              </p>
            </CardTitle>
            <PrivateAuditEdit private_audit={privateAudit} />
          </div>
        </CardHeader>
        <Separator style={{ backgroundColor: privateAudit.secondary_color }} />
                  ...
                  <div className="space-y-2">
                    ...
                    <dd className="flex items-center gap-3">
                      <div
                        ...
                        style={{ backgroundColor: privateAudit?.primary_color }}
                      />
                      <span className="text-sm font-mono font-medium text-foreground uppercase">
                        {privateAudit?.primary_color || '—'}
                      </span>
                    </dd>
                  </div>
                  <div className="space-y-2">
                    ...
                    <dd className="flex items-center gap-3">
                      <div
                        className="w-12 h-12 rounded-lg border-2 border-border shadow-sm"
                        style={{
                          backgroundColor: privateAudit?.secondary_color
                        }}
                      />
                      <span className="text-sm font-mono font-medium text-foreground uppercase">
                        {privateAudit?.secondary_color || '—'}
                      </span>
                    </dd>
                  </div>
                </div>
