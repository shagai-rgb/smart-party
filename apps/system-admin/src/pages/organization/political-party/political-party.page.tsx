...
import { PoliticalPartyEdit } from './components/political-party-edit.dialog'

export default function PoliticalParty() {
  const { id } = useParams()
  const organization = organizationMock.find((org) => org.id === Number(id))
      ...
      <Breadcrumb className="mb-4">
        <BreadcrumbList>
          ...
          <BreadcrumbItem className="uppercase">
            <BreadcrumbPage>{organization.full_name}</BreadcrumbPage>
          </BreadcrumbItem>
        </BreadcrumbList>
      </Breadcrumb>
        ...
        <CardHeader className=" flex items-center font-medium  ">
          <div className="flex items-center justify-between w-full ">
            <CardTitle className="text-xl  flex items-center gap-1.5 uppercase  font-medium ">
              ...
              />
              <p style={{ color: organization.primary_color }}>
                {organization?.full_name}
              </p>
            </CardTitle>
            <PoliticalPartyEdit organization={organization} />
          </div>
        </CardHeader>
        <Separator style={{ backgroundColor: organization.secondary_color }} />
